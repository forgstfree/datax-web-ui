<template>
  <div class="graph-container" id="save-img">
    ...
    >
      <g id="save-img-svg-g" :style="{ cursor: 'grab' }"></g>
    </svg>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      // innerUrl: 'http://10.9.21.98:13333/media/result/CommonER.json',
      // innerUrl: 'http://10.9.21.4:13333/media/result/EnhanceER.json',
      innerUrl: "http://10.9.21.4:13333/media/result/50test.log.json",
      // innerUrl: 'http://10.9.21.98:13333/media/result/100ubuntu_R8tgeIs.log.json',
      innerWidth: 400,
      innerHeight: 300,
    };
  },
  mounted() {
    ...
    function network(data, prev, index, collapse, nodeScale) {
      let i;
      let l;
      let k;
      collapse = collapse || {};
    ...
    function init(
      ...
      const off = 5;
      let collapse = {};
      const fontScale = d3.scale
        .linear()
        .domain([nodeMinTime, nodeMaxTime])
        .range([6, 18]);
      const linkScale = d3.scale
        .linear()
        .domain([linkMinTime, linkMaxTime])
        .range([1, 5 + linkMaxTime / 3]);
      const nodeScale = d3.scale
        .linear()
        .domain([nodeMinTime, nodeMaxTime])
        .range([8, 20 + nodeMaxTime / 2]);

      if (force) force.stop();
      net = network(data, net, getGroup, collapse, fontScale);
      ...
      let link = linkG.selectAll("line.link").data(net.links, linkId);
      link.exit().remove();
      link
        .enter()
        .append("line")
      ...
      let node = nodeG.selectAll("circle.node").data(net.nodes, nodeId);
      node.exit().remove();
      node
        .enter()
        .append("circle")
      ...
      let nodeLabel = nodeG.selectAll("text.node").data(net.nodes, nodeId);
